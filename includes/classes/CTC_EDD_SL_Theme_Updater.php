 ...
 * - $update_onclick message, textdomain
 * - update-nag message, localized
 * - $update_data['url'] if not set to prevent warnings
...
class CTC_EDD_SL_Theme_Updater {
	...
	function update_nag() {
		...
		$update_onclick = ' onclick="if ( confirm(\'' . esc_js( sprintf( __( "Any customizations you have made directly to %s's theme files will be lost. Please consider using a child theme for modifications.", "ct-framework" ), $theme->get( 'Name' ) ) ) . '\') ) {return true;}return false;"';
	...
	function check_for_update() {
		...
		if ( false === $update_data ) {
			...
			// CT Mod: failure if no new_version (sometimes only returns valid object with msg indicating failure)
			...
			if ( ! $failed ) {
				...
				// CT Mod: Prevent "Undefined index: url"
				$update_data->url = isset( $update_data->url ) ? $update_data->url : '';
